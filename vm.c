...
static void resetStack() {
  vm.stack->stackTop = vm.stack->array;
  vm.frameCount = 0;
}
...
static void runtimeError(const char *format, ...) {
  ...
  for (int i = vm.frameCount - 1; i >= 0; i--) {
    ...
    int lineNumber = getLine(&function->chunk.lines, instructionIndex);
    fprintf(stderr, "[line %d] in ", lineNumber);
...
static bool callValue(Value callee, int argCount) {
  if (IS_OBJECT(callee)) {
    ...
    case OBJECT_NATIVE: {
      ...
      Value result = native(argCount, vm.stack->stackTop - argCount);
      vm.stack->stackTop -= argCount + 1;
      push(result);
      return true;
    }
    default: {
...
#define READ_LONG()                                                            \
  (frame->ip += 3,                                                             \
   (uint16_t)((frame->ip[-3] << 16) | (frame->ip[-2] << 8) | frame->ip[-1]))
